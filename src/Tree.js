...
function generateNodes(report, TTP, file, registry) {
  ...
  if (TTP) {
    selected_pids = selected_pids.filter((pid) =>
    {
      ...
      report["processes"][pid]["ttps"]
        .forEach((ttp) => {ttp["att&ck"].forEach((attck_name) => {ttps_set.add(attck_name)})});
...
export default function ProcessTree({
  ...
  const nodes = generateNodes(report, selectedTTP, selectedFile, selectedRegistry);
